...
程序窗口打开（占桌面 80%，居中），同时**自动弹出一个浏览器窗口**。

浏览器窗口里是微博登录页，用手机微博 App **扫码登录**。

登录后回到程序窗口，点上方按钮操作。

> 弹出的浏览器是本程序**自带的**，和系统里装的 Edge、Chrome 无关，也不需要系统有浏览器。
...
| 全部删除 | 删掉所有未删除的微博。删完会**自动清缓存并复核一次**，确保无遗漏 |
| 先删 3 条试试 | 只删 3 条，用来验证工具能用 |
