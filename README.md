...
> **重要：删掉的微博找不回来。** 建议先用「看看有多少条」，再用「先删 3 条试试」确认能用，最后才正式删。
...
> 解压！不要直接双击 ZIP 里面的 exe，那样用不了。要先把整个文件夹解压出来。

## 2. 打开

双击 WeiboDelete.exe。

窗口会占满屏幕的 80%，居中显示。上方是微博网页，下方是运行日志。

第一次运行会自动弹出微博登录页，用手机微博 App **扫码登录**。
...
| 全部删除 | 删掉账号里所有未删除的微博，会二次确认 |
| 先删 3 条试试 | 只删 3 条，用来验证工具能不能用 |
...
Windows 10 1803 以上，或 Windows 11。

系统自带 **Microsoft Edge WebView2 Runtime**（绝大多数电脑都有，Win11 一定自带）。

如果打开时提示缺少内核，按弹窗点「是」会打开微软官方下载页，装好后重新运行即可。这一步只需做一次。

## 5. 运行中生成的文件

```
data\       删除进度、日志
profile\    登录状态（含 cookie，别外传）
```
...
**Q：删错了能恢复吗？**
**不能。** 微博删了就没了。所以一定要先用「看看有多少条」和「先删 3 条试试」确认。
...
|---|---|---|
| 位置 | `win32/` | 仓库根目录 |
| 体积 | 832 KB | 依赖 150MB Chromium |
| 依赖 | 系统自带 WebView2 | 需 Python 3.9+ |
| 浏览器内核 | 系统 Edge | Playwright 自带 Chromium |
